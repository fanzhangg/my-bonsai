...
test('anonymous claim is durable and idempotent; old links work and joining remains disabled',async()=>{
 ...
 try{const id=randomUUID(),first=await request('',{id});assert.equal(first.status,201);assert.deepEqual(first.data.config,configForClaim(id));assert(!('participants' in first.data));assert.equal(first.data.name,'');const again=await request('',{id});assert.deepEqual(first.data.config,again.data.config);assert.equal(first.data.createdAt,again.data.createdAt);assert.equal((await request('/'+id)).data.id,id);assert.equal((await request('/'+id+'/cuts',{})).status,400);assert.equal((await request('/'+id+'/join',{})).status,410);assert.equal((await request('/bad')).status,404);const reopened=await openStore({url:'',file});assert.equal((await reopened.get(id)).id,id);await reopened.close();
