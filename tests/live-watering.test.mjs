...
test('partial and full watering persist, replay consistently, and retries never duplicate growth',async()=>{
 ...
 try{
  ...
  assert.ok(Math.abs(snapshot(after,at).progress-snapshot(before,at).progress-.065)<1e-10);
  ...
  assert.equal(recovery.waterings.at(-1).recoveryHours,12);
  ...
  assert.equal(cheat.waterings.at(-1).recoveryHours,12);
...
test('watering a bare mature tree visibly extends new shoots without restoring cut branches',()=>{
 for(const {id:preset} of PRESETS){
  ...
  const watered={...bare,waterings:[{at:at+1,amount:.05,recoveryHours:wateringRecovery(before,4000)}]};
  const full=snapshot(watered,at+1);
  assert.ok(full.nodes.some(n=>n.regrown&&Math.hypot(n.ex-n.x,n.ey-n.y)>2),preset);
  assert.ok(!full.nodes.some(n=>cutIds.includes(n.id)));
  assert.deepEqual(full.nodes.filter(n=>!n.regrown),before.nodes);
