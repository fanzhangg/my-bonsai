    ...
    return {
      async listGallery(sort='active',{after=null,query='',limit=GALLERY_PAGE_SIZE}={}){
        ...
        const {rows}=await pool.query(`SELECT document FROM (
          ...
          FROM bonsai_trees WHERE strpos(lower(COALESCE(document->>'name','')),lower($4::text))>0
        ) activity WHERE ${key}>0 AND ($2::bigint IS NULL OR ${key}<$2 OR (${key}=$2 AND id>$3::uuid)) ORDER BY ${key} DESC,id ASC LIMIT $1`,[limit,after?.at??null,after?.id??null,query]);
