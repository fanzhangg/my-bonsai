...
const pruning=createPruning({
 ...
 onBusyChange:busy=>{wind.setPaused(busy);visitors.setActive(!busy);$('debug').querySelectorAll('button,input,select').forEach(el=>el.disabled=busy);},
 onCommit:async branchId=>{
  if(sandbox){sandbox.cuts.push({id:crypto.randomUUID(),seq:sandbox.cuts.length+1,at:now(),branchId});return;}
...
async function sync(){if(loading||playing||pruning.busy||(!treeId)||sandbox)return;loading=true;pruningAvailability();try{const data=await api('/'+treeId),initial=!record;record=data;offset=data.serverNow-Date.now();$('copy').hidden=false;$('retry').hidden=true;status();if(debug){resetSandbox();$('debug').hidden=false;}if(initial)replay();else paint();}catch(e){status(e.message||'暂时无法连接');$('retry').hidden=false;}finally{loading=false;pruningAvailability();}}
$('retry').onclick=sync;
