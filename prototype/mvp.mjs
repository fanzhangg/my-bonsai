...
import {colorsFor,LOOKS,lookFor} from './core/v1/appearance.mjs';
...
function paint(tree=snapshot(current(),now())){const c=colorsFor(tree.config.appearance);document.documentElement.style.background=c.background;document.body.classList.toggle('night',tree.config.appearance.background==='night');document.querySelector('meta[name="theme-color"]').content=c.background;$('stage').innerHTML=draw(tree);if(debug){$('age').textContent=`${hours.toFixed(1)} 小时`;$('info').textContent=`${record.id||'未认领样本'}\n${VERSION}\n成熟需 ${tree.matureDays.toFixed(1)} 天 · 生长 ${(tree.progress*100).toFixed(1)}% · 枝段 ${tree.nodes.length} · 叶团 ${tree.clusters.filter(c=>tree.hour>c.born).length}\n${playing?'回放中':'静止预览'}`;}}
