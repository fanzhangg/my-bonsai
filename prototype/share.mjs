...
export function createSharing({button,getRecord,onStatus}){
 ...
 async function prepare(){
  ...
  const data=shareData(record,location.origin,Date.now(),language);
  ...
  pending=(async()=>{
   try{
    ...
    if(!controller.signal.aborted)file=new File([blob],t('我的盆栽.png'),{type:'image/png'});
 ...
 button.onclick=async()=>{
  ...
  const data=shareData(record,location.origin,Date.now(),language),image=fileKey===data.image?file:null;
  ...
  try{
   if(typeof navigator.share==='function'){
    ...
    onStatus(t('邀请文案和盆栽链接已复制，发给朋友来浇水吧。'));
   }
  }catch(error){
   if(error.name!=='AbortError')onStatus(typeof navigator.share==='function'?t('暂时无法打开系统分享，请稍后重试。'):t('未能复制邀请，请从地址栏复制盆栽链接发给朋友。'));
