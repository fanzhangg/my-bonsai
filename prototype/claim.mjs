...
export function configForClaim(id,version=LEGACY_VERSION){
  treeVersion({version});
  if(version===CURRENT_VERSION){
    const pick=(items,key)=>items[Math.floor(sample(id,'claim-v3',key)*items.length)];
    const form=pick(FORMS,'form');
    return normalizeDesign({seed:id,preset:form.id,crown:pick(form.crowns,'crown'),leaf:pick(form.leaves,'leaf'),palette:pick(Object.keys(PALETTES),'palette'),
