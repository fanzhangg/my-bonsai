...
import {generate as morphology,canopyPoint} from './morphology.mjs';
...
export const LEAVES={scale:'短鳞叶',needle:'针叶束',oval:'椭圆叶',round:'圆叶',maple:'掌状叶',fan:'扇形叶'};
...
const tint=(hex,amount)=>{
  const target=amount>0?[242,239,205]:[23,38,28];
...
export function generateLanguage(input={}){
  ...
  for(const c of tree.clusters){
    ...
    if(tree.individual||form.id!=='juniper'||palette!=='forest'){
      ...
      c.layerPalette=PALETTES[palette].layers[c.crownLayer??1].map(color=>tint(color,tone));
    }
