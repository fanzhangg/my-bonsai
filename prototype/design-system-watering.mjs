...
export function createWateringReview(){
 const $=id=>document.getElementById(id),scene=$('watering-scene'),holder=$('watering-tree'),can=$('watering-can'),water=$('watering-water'),status=$('watering-status');
 ...
 let progress=.55,tree,svg,active=false,held=false,pouring=false,returning=false,pointer=null,raf=0,started=0,lastEmit=0,lastGrowth=0,lastFrame=0,direction=1,growthTarget=.55,lastSplash=0,pouredThisHold=false,position={x:0,y:0};
 const particles=[],splashes=[];
 const tank=createWaterTank();
 const reduced=()=>matchMedia('(prefers-reduced-motion: reduce)').matches;
 function paint(value){progress=value;tree=grow(record,value);holder.innerHTML=render(tree,{hour:tree.hour,transparent:true,id:'watering-sample',viewBox:{x:-50,y:10,width:600,height:580}});svg=holder.querySelector('svg');holder.dataset.progress=value.toFixed(4);}
 paint(progress);
 ...
 function home(){return {x:scene.clientWidth-66,y:scene.clientHeight-70};}
 ...
 async function returnHome(){
  ...
  place(home());refillWaterTank(tank);pouredThisHold=false;showWater();status.textContent='水壶已补满，可以再次拿起。';returning=false;
 }
 function complete(){
 ...
 function begin(){if(!active||returning||held||pouring)return false;held=true;pouredThisHold=pouring;can.classList.add('is-held');status.textContent=pouring?'继续移动水壶，水流会跟随壶嘴。':'移到盆栽或花盆附近即可浇水。';return true;}
 function move(e){
 ...
 function release(){if(!held)return;held=false;pointer=null;can.classList.remove('is-held','is-pouring','is-draining');water.dataset.phase='idle';water.dataset.flowing='false';returnHome();}
 ...
 return {setActive(value){active=value;if(!active){release();if(pouring)complete();}else{if(!held&&!pouring&&!returning)place(home());}}};
}
