...
export function createPruningReview(){
  const $=id=>document.getElementById(id),scene=$('pruning-scene'),tool=$('pruning-scissors'),message=$('pruning-message');
  ...
  const svg=$('pruning-tree').querySelector('svg'),ns='http://www.w3.org/2000/svg';
  ...
  function begin(){if(!active||busy||held)return false;held=true;tool.classList.add('is-held');scene.classList.add('is-dragging');message.textContent='慢慢来，靠近一根向外伸展的旁支。';return true;}
  function move(e){
  ...
  return {setActive(value){active=value;if(!active)finish(false);else if(!held&&!busy){const h=home();position(h.x,h.y);}}};
}
