...
export function normalizeDesign(input={}){
  ...
  return {...normalize(input),preset:form.id,crown,leaf,palette,
    ...
    ...(NATURAL_GROWTH_POLICIES.includes(input.growthPolicy)?{growthPolicy:input.growthPolicy}:{}),
...
export function validateDesign(input){
  ...
  if((input.growthPolicy!==undefined&&!NATURAL_GROWTH_POLICIES.includes(input.growthPolicy))||!form||!form.crowns.includes(input.crown)||!form.leaves.includes(input.leaf)||!Object.hasOwn(PALETTES,input.palette)||
