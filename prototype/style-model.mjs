...
export const VERSION = 'bonsai-style-2';
...
export function normalizeConfig(input={}) {
  return {...DEFAULTS,algorithmVersion:VERSION,seed:String(input.seed??DEFAULTS.seed).slice(0,64),style:STYLES.includes(input.style)?input.style:'moyogi',
...
export class StyleSimulation {
  ...
  candidate(parent,index,slot){
    ...
    // Scaffold wood may hang down; new shoots retain an outward, upward flow.
    // The crown is a boundary, never a random point that attracts tips backward.
    const side=parent.flowSide??pad?.side??(Math.sin(parent.angle)>=0?1:-1);
    const tangent=Math.atan2(parent.ex-parent.cx2,parent.cy2-parent.ey);
    const envelope=pad??{x:start.x+side*28,y:start.y-20,rx:42,ry:32};
    const leader=slot.leader||parent.leader;
    const incoming=parent.role==='twig'?clamp(side*tangent,.16,1.42):1.15;
    const guide=clamp(incoming+(index===0?-.035:-.32)-(leader?.12:0),.16,1.42)*side;
    const jitter=(this.rand(key,'angle')-.5)*.1*this.config.randomness;
    ...
    const siblingAngles=this.nodes.filter(n=>n.alive&&n.parent===parent.id&&n.role==='twig').map(n=>n.angle);
    const distance=q=>Math.hypot((q.x-envelope.x)/envelope.rx,(q.y-envelope.y)/envelope.ry);
    const startDistance=distance(start);
    let best=null;
    for(const scale of [1,.72,.48])for(const turn of [0,-.16,.16,-.32,.32]){
      const angle=clamp(side*(guide+turn+jitter),.16,1.42)*side;
      if(parent.role==='twig'&&Math.abs(angle-tangent)>.56)continue;
      if(siblingAngles.some(a=>Math.abs(a-angle)<.2))continue;
      const actualLength=length*scale,end={x:start.x+Math.sin(angle)*actualLength,y:start.y-Math.cos(angle)*actualLength};
      if(this.config.guidance&&distance(end)>Math.max(1.15,startDistance-.08))continue;
      ...
      const curve=this.shootCurve(start,end,key);
      if(this.crossesWood(curve,parent.id))continue;
      const guideScore=Math.cos(angle-guide);
      const score=guideScore*.8+(this.config.guidance?-outside*1.2-gap*.7:0)+env.light*.25+env.room*.4+env.upward*.2+scale*.14;
      if(!best||score>best.score)best={...env,score,angle,length:actualLength,end,outside,gap,curve,side};
    }
    return best;
  }
  shootCurve(start,end,key){
    // Ordered control points keep the entire cubic upward/outward, including
    // its tangent. Endpoint-only angle checks still allow dangling hooks.
    const k=.3+(this.rand(key,'bend')-.5)*.09*this.config.randomness;
    return {x:start.x,y:start.y,cx1:start.x+(end.x-start.x)*k,cy1:start.y+(end.y-start.y)*.25,
      cx2:start.x+(end.x-start.x)*.72,cy2:start.y+(end.y-start.y)*.78,ex:end.x,ey:end.y};
  }
  crossesWood(curve,parentId){
    const points=Array.from({length:7},(_,i)=>pointOn(curve,i/6));
    const bounds=n=>({minX:Math.min(n.x,n.cx1,n.cx2,n.ex),maxX:Math.max(n.x,n.cx1,n.cx2,n.ex),minY:Math.min(n.y,n.cy1,n.cy2,n.ey),maxY:Math.max(n.y,n.cy1,n.cy2,n.ey)});
    const a=bounds(curve),orient=(p,q,r)=>(q.x-p.x)*(r.y-p.y)-(q.y-p.y)*(r.x-p.x);
    for(const n of this.nodes){
      if(!n.alive||n.id===parentId)continue;
      const b=bounds(n);if(a.maxX<b.minX||a.minX>b.maxX||a.maxY<b.minY||a.minY>b.maxY)continue;
      let prev=pointOn(n,0);
      for(let j=1;j<=6;j++){
        const next=pointOn(n,j/6);
        for(let i=1;i<points.length;i++)if(orient(points[i-1],points[i],prev)*orient(points[i-1],points[i],next)<-1e-7&&orient(prev,next,points[i-1])*orient(prev,next,points[i])<-1e-7)return true;
        prev=next;
      }
    }
    return false;
  ...
  tick(hour){
    ...
    for(const n of living){
      for(let i=0;i<n.slots.length;i++){
        ...
        const c=this.candidate(n,i,s);if(!c){s.diagnostics={blocked:true,light:0,room:0,weight:0};continue;}
        const apical=i>0&&n.slots[0]?.child?1-this.config.dominance*.65:1;
    ...
    for(const {n,i,s,c} of candidates.slice(0,Math.min(3,this.maxNodes-living.length))){
      if(this.crossesWood(c.curve,n.id))continue;
      if(this.nodes.some(other=>other.alive&&other.parent===n.id&&other.role==='twig'&&Math.abs(other.angle-c.angle)<.2))continue;
      ...
      const child=this.add({id,key:s.key,parent:n,start,end,
        c1:{x:c.curve.cx1,y:c.curve.cy1},c2:{x:c.curve.cx2,y:c.curve.cy2},
      ...
      child.slotIndex=i;child.flowSide=c.side;child.leader=s.leader===true||(n.leader&&i===0);s.child=id;
